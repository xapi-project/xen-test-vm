...
type action = Suspend | PowerOff | Reboot | Halt | Crash | Ignore
...
(** message to a guest *)
type t = Now of action | OnShutdown of ack * action
...
let from_string str =
  try
    ...
    json |> U.member "when" |> U.to_string |> do_when ack' action'
  with Yojson.Json_error msg -> error "bad json: %s" msg
